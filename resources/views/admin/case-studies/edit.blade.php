...
<div class="bg-white rounded-lg shadow">
    <form method="POST" action="{{ route('admin.content.case-studies.update', $caseStudy->id) }}" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="image" id="image" value="{{ old('image', $caseStudy->image) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            placeholder="https://example.com/image.jpg">
                        @if($caseStudy->image || old('image'))
                        <div class="mt-2 h-48 w-full rounded-md border border-gray-200 bg-gray-100 overflow-hidden">
                            <img src="{{ old('image', $caseStudy->image) }}" alt="Preview" id="image-preview" class="w-full h-full object-cover" onerror="this.style.display='none'">
                        </div>
                        @endif
                    </div>
                </div>
